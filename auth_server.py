...
def prompt_for_credentials(set_user=True):
    if set_user:
        ...
        print(f"[OK] Updated AUTH_USERNAME = {user}")
    ...
    print("[OK] Password hash updated in .env")
    sys.exit(0)
...
def main():
    ...
    if not os.path.isdir(base_path):
        print(f"[ERROR] Error: directory '{base_path}' not found or is not a directory.")
        sys.exit(1)
    ...
    if not os.path.exists(public_path):
        try:
            ...
            print(f"[DIR] Created missing public directory: {public_path}")
        except Exception as e:
            print(f"[ERROR] Failed to create public directory '{public_path}': {e}")
            sys.exit(1)
    elif not os.path.isdir(public_path):
        print(f"[ERROR] Error: '{public_path}' exists but is not a directory.")
        sys.exit(1)
    ...
    print(f"[OK] Serving directory: {base_path}")
    print(f"[OK] Public subdirectory: /{args.public_dir}")
    print(f"[OK] Listening on port: {args.port}")
    print(f"[MODE] Transfer method: {TRANSFER_METHOD}")
